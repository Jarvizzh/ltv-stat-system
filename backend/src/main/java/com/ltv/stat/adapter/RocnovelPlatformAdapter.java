...
/**
 * 中文在线 (Rocnovel / Florastory) 专属适配器实现
 */
@Component
public class ZhongwenOnlinePlatformAdapter implements PlatformSyncAdapter {

    private static final Logger log = LoggerFactory.getLogger(ZhongwenOnlinePlatformAdapter.class);
    private static final DateTimeFormatter DATE_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd");

    private final OrderSyncService orderSyncService;
    private final SubscribeConfigSyncService subscribeConfigSyncService;

    public ZhongwenOnlinePlatformAdapter(@Lazy OrderSyncService orderSyncService,
    ...
    public int syncOrders(LocalDate startDate, LocalDate endDate, PlatformConfig config) {
        ...
        log.info("[ZhongwenOnlineAdapter] Triggering syncOrders from {} to {}", startStr, endStr);
    ...
    public int syncOrdersForLandingPage(String landingPageId, PlatformConfig config) {
        log.info("[ZhongwenOnlineAdapter] Triggering syncOrdersForLandingPage: {}", landingPageId);
        return orderSyncService.syncOrdersForLandingPage(landingPageId);
    }

    @Override
    public int syncConfigs(PlatformConfig config) {
        log.info("[ZhongwenOnlineAdapter] Triggering syncAllSubscribeConfigs...");
